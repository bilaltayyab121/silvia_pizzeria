  ...
  return (
    <section id="location" className="mt-20 grid gap-10 lg:grid-cols-[1.1fr_0.9fr] lg:items-start">
      <div className="rounded-[40px] border border-slate-200 bg-white p-6 shadow-soft sm:p-10">
        <SectionTitle 
          number="04" 
          title={language === 'fr' ? "Emplacement & Horaires" : 'Location & Hours'} 
          description={language === 'fr' ? "Nous sommes au Casino plage, 127, Témara." : 'Find us at Casino plage, 127, Témara. Open Tuesday through Sunday for lunch and dinner.'} 
        />
      ...
      <div className="space-y-6 rounded-[40px] border border-slate-200 bg-italian-cream p-8 shadow-soft">
        <div>
          <h3 className="font-heading text-2xl font-semibold text-slate-900">{language === 'fr' ? 'Horaires' : 'Opening Hours'}</h3>
          <p className="mt-3 text-sm text-slate-700">{language === 'fr' ? "Fermé le lundi." : 'Closed Mondays. Open every other day from noon to midnight.'}</p>
        </div>
