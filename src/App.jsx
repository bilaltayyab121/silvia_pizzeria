...
import { buildWhatsAppLinkFromCart } from "./utils";
...
function App() {
  ...
  function addToCart(item, quantity = 1) {
    setCart((c) => {
      const exists = c.find((it) => it.id === item.id);
      if (exists)
        return c.map((it) =>
  ...
  function sendOrder() {
    ...
    const url = buildWhatsAppLinkFromCart(cart, language, whatsappNumber);
      ...
      <main className="mx-auto max-w-6xl px-6 py-6 md:px-8">
        ...
        <div className="mt-24">
          <SectionTitle
            ...
            description={
              language === "fr"
                ? "Parcourez nos pizzas, pâtes, salades et boissons."
                : "Browse our pizza, pasta, salad, and beverage choices."
            }
          />
          <Menu
